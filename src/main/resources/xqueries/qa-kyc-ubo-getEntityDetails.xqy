...
let $stockExchangeRelationship := qa-kyc-ubo-getRelationshipDocs:getRelationships($fid, "exchange member", "stock exchange")
...
let $stockExchangeAbbrName := if(fn:exists($stockExchangeDoc/summary/names/name[type="Abbreviated Title"]/value))
                              then <abbr> ({$stockExchangeDoc/summary/names/name[type="Abbreviated Title"]/value/text()})</abbr>
                              else ()
...
let $stockExchangeDoc := qa-kyc-ubo-getLegalEntityDoc:getLegalEntityDocByResource($x/parties/party[partyType = "stock exchange"]/entityReference/link/@href)
let $stockExchangeAbbrName := $stockExchangeDoc/summary/names/name[type="Abbreviated Title"]/value/text()
...
let $regulationRelationship := qa-kyc-ubo-getRelationshipDocs:getRelationships($fid, "regulated", "regulation")
